


```
https://api.hh.ru/vacancies?professional_role=96&area=113&date_from=2025-08-19&date_to=2025-08-19&per_page=50&page=22
```
```professional_role=96``` - id проф роли, тут IT
```area=113``` - id Страны (Россия)
```date_from=2025-08-19``` - дата от какого числа брать выборку
```date_to=2025-08-19``` - дата до какого числа брать выборку
```per_page=100``` - сколько элементов на странице за раз
```page=22``` - номер страницы

api
found: 1101,
pages: 12,
page: 0,
per_page: 100,
clusters: null,
arguments: null,
fixes: null,
suggests: null,

[professional_role](https://api.hh.ru/professional_roles)

```json
{
  "categories":[
    {
      "id":"19",
      "name":"Автомобильный бизнес",
      "roles":[
        {
          "id":"4",
          "name":"Автомойщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"5",
          "name":"Автослесарь, автомеханик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"62",
          "name":"Мастер-приемщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"70",
          "name":"Менеджер по продажам, менеджер по работе с клиентами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"5",
      "name":"Административный персонал",
      "roles":[
        {
          "id":"8",
          "name":"Администратор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"33",
          "name":"Делопроизводитель, архивариус",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"58",
          "name":"Курьер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"76",
          "name":"Менеджер/руководитель АХО",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"84",
          "name":"Оператор ПК, оператор базы данных",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"88",
          "name":"Офис-менеджер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"93",
          "name":"Переводчик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"110",
          "name":"Секретарь, помощник руководителя, ассистент",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"15",
      "name":"Безопасность",
      "roles":[
        {
          "id":"22",
          "name":"Военнослужащий",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"90",
          "name":"Охранник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"95",
          "name":"Полицейский",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"116",
          "name":"Специалист по информационной безопасности",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"120",
          "name":"Специалист службы безопасности",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"26",
      "name":"Высший и средний менеджмент",
      "roles":[
        {
          "id":"26",
          "name":"Генеральный директор, исполнительный директор (CEO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"36",
          "name":"Директор по информационным технологиям (CIO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"37",
          "name":"Директор по маркетингу и PR (CMO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"38",
          "name":"Директор по персоналу (HRD)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"166",
          "name":"Директор юридического департамента (CLO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"53",
          "name":"Коммерческий директор (CCO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"80",
          "name":"Начальник производства",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"87",
          "name":"Операционный директор (COO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"157",
          "name":"Руководитель отдела аналитики",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"172",
          "name":"Руководитель отдела логистики",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"170",
          "name":"Руководитель отдела маркетинга и рекламы",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"171",
          "name":"Руководитель отдела персонала",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"161",
          "name":"Руководитель филиала",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"125",
          "name":"Технический директор (CTO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"135",
          "name":"Финансовый директор (CFO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"8",
      "name":"Добыча сырья",
      "roles":[
        {
          "id":"27",
          "name":"Геодезист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"28",
          "name":"Геолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"168",
          "name":"Лаборант",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"79",
          "name":"Научный специалист, исследователь",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"82",
          "name":"Начальник смены, мастер участка",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"49",
          "name":"Технолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"16",
      "name":"Домашний, обслуживающий персонал",
      "roles":[
        {
          "id":"8",
          "name":"Администратор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"21",
          "name":"Водитель",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"23",
          "name":"Воспитатель, няня",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"32",
          "name":"Дворник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"58",
          "name":"Курьер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"89",
          "name":"Официант, бармен, бариста",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"90",
          "name":"Охранник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"130",
          "name":"Уборщица, уборщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"14",
      "name":"Закупки",
      "roles":[
        {
          "id":"66",
          "name":"Менеджер по закупкам",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"119",
          "name":"Специалист по тендерам",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"11",
      "name":"Информационные технологии",
      "roles":[
        {
          "id":"156",
          "name":"BI-аналитик, аналитик данных",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"160",
          "name":"DevOps-инженер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"10",
          "name":"Аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"12",
          "name":"Арт-директор, креативный директор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"150",
          "name":"Бизнес-аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"25",
          "name":"Гейм-дизайнер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"165",
          "name":"Дата-сайентист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"34",
          "name":"Дизайнер, художник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"36",
          "name":"Директор по информационным технологиям (CIO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"73",
          "name":"Менеджер продукта",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"155",
          "name":"Методолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"96",
          "name":"Программист, разработчик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"164",
          "name":"Продуктовый аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"104",
          "name":"Руководитель группы разработки",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"157",
          "name":"Руководитель отдела аналитики",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"107",
          "name":"Руководитель проектов",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"112",
          "name":"Сетевой инженер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"113",
          "name":"Системный администратор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"148",
          "name":"Системный аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"114",
          "name":"Системный инженер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"116",
          "name":"Специалист по информационной безопасности",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"121",
          "name":"Специалист технической поддержки",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"124",
          "name":"Тестировщик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"125",
          "name":"Технический директор (CTO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"126",
          "name":"Технический писатель",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"24",
      "name":"Искусство, развлечения, массмедиа",
      "roles":[
        {
          "id":"12",
          "name":"Арт-директор, креативный директор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"13",
          "name":"Артист, актер, аниматор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"20",
          "name":"Видеооператор, видеомонтажер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"25",
          "name":"Гейм-дизайнер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"34",
          "name":"Дизайнер, художник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"41",
          "name":"Журналист, корреспондент",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"55",
          "name":"Копирайтер, редактор, корректор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"98",
          "name":"Продюсер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"103",
          "name":"Режиссер, сценарист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"139",
          "name":"Фотограф, ретушер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"6",
      "name":"Маркетинг, реклама, PR",
      "roles":[
        {
          "id":"1",
          "name":"Event-менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"2",
          "name":"PR-менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"3",
          "name":"SMM-менеджер, контент-менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"10",
          "name":"Аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"12",
          "name":"Арт-директор, креативный директор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"34",
          "name":"Дизайнер, художник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"37",
          "name":"Директор по маркетингу и PR (CMO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"55",
          "name":"Копирайтер, редактор, корректор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"163",
          "name":"Маркетолог-аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"68",
          "name":"Менеджер по маркетингу, интернет-маркетолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"70",
          "name":"Менеджер по продажам, менеджер по работе с клиентами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"71",
          "name":"Менеджер по работе с партнерами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"99",
          "name":"Промоутер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"170",
          "name":"Руководитель отдела маркетинга и рекламы",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"23",
      "name":"Медицина, фармацевтика",
      "roles":[
        {
          "id":"8",
          "name":"Администратор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"15",
          "name":"Ассистент врача",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"19",
          "name":"Ветеринарный врач",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"24",
          "name":"Врач",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"29",
          "name":"Главный врач, заведующий отделением",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"42",
          "name":"Заведующий аптекой",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"168",
          "name":"Лаборант",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"64",
          "name":"Медицинская сестра, медицинский брат",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"65",
          "name":"Медицинский представитель",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"79",
          "name":"Научный специалист, исследователь",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"151",
          "name":"Специалист по сертификации",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"133",
          "name":"Фармацевт-провизор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"25",
      "name":"Наука, образование",
      "roles":[
        {
          "id":"17",
          "name":"Бизнес-тренер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"23",
          "name":"Воспитатель, няня",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"168",
          "name":"Лаборант",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"167",
          "name":"Методист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"79",
          "name":"Научный специалист, исследователь",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"101",
          "name":"Психолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"132",
          "name":"Учитель, преподаватель, педагог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"1",
      "name":"Продажи, обслуживание клиентов",
      "roles":[
        {
          "id":"6",
          "name":"Агент по недвижимости",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"10",
          "name":"Аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"154",
          "name":"Брокер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"51",
          "name":"Кассир-операционист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"53",
          "name":"Коммерческий директор (CCO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"54",
          "name":"Координатор отдела продаж",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"57",
          "name":"Кредитный специалист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"70",
          "name":"Менеджер по продажам, менеджер по работе с клиентами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"71",
          "name":"Менеджер по работе с партнерами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"83",
          "name":"Оператор call-центра, специалист контактного центра",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"97",
          "name":"Продавец-консультант, продавец-кассир",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"105",
          "name":"Руководитель отдела клиентского обслуживания",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"106",
          "name":"Руководитель отдела продаж",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"161",
          "name":"Руководитель филиала",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"151",
          "name":"Специалист по сертификации",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"121",
          "name":"Специалист технической поддержки",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"122",
          "name":"Страховой агент",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"129",
          "name":"Торговый представитель",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"7",
      "name":"Производство, сервисное обслуживание",
      "roles":[
        {
          "id":"174",
          "name":"Инженер ПНР",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"44",
          "name":"Инженер по качеству",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"45",
          "name":"Инженер по охране труда и технике безопасности, инженер-эколог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"46",
          "name":"Инженер по эксплуатации",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"48",
          "name":"Инженер-конструктор, инженер-проектировщик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"169",
          "name":"Инженер-электроник, инженер-электронщик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"144",
          "name":"Инженер-энергетик, инженер-электрик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"149",
          "name":"Контролёр ОТК",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"168",
          "name":"Лаборант",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"162",
          "name":"Мастер по ремонту оборудования, техники",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"152",
          "name":"Метролог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"173",
          "name":"Механик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"79",
          "name":"Научный специалист, исследователь",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"80",
          "name":"Начальник производства",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"82",
          "name":"Начальник смены, мастер участка",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"85",
          "name":"Оператор производственной линии",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"86",
          "name":"Оператор станков с ЧПУ",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"109",
          "name":"Сварщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"111",
          "name":"Сервисный инженер, инженер-механик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"115",
          "name":"Слесарь, сантехник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"151",
          "name":"Специалист по сертификации",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"49",
          "name":"Технолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"128",
          "name":"Токарь, фрезеровщик, шлифовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"141",
          "name":"Швея, портной, закройщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"143",
          "name":"Электромонтажник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"17",
      "name":"Рабочий персонал",
      "roles":[
        {
          "id":"5",
          "name":"Автослесарь, автомеханик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"21",
          "name":"Водитель",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"31",
          "name":"Грузчик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"52",
          "name":"Кладовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"59",
          "name":"Маляр, штукатур",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"173",
          "name":"Механик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"78",
          "name":"Монтажник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"85",
          "name":"Оператор производственной линии",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"86",
          "name":"Оператор станков с ЧПУ",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"102",
          "name":"Разнорабочий",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"109",
          "name":"Сварщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"111",
          "name":"Сервисный инженер, инженер-механик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"115",
          "name":"Слесарь, сантехник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"128",
          "name":"Токарь, фрезеровщик, шлифовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"131",
          "name":"Упаковщик, комплектовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"143",
          "name":"Электромонтажник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"2",
      "name":"Розничная торговля",
      "roles":[
        {
          "id":"9",
          "name":"Администратор магазина, администратор торгового зала",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"35",
          "name":"Директор магазина, директор сети магазинов",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"77",
          "name":"Мерчандайзер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"97",
          "name":"Продавец-консультант, продавец-кассир",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"99",
          "name":"Промоутер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"123",
          "name":"Супервайзер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"127",
          "name":"Товаровед",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"9",
      "name":"Сельское хозяйство",
      "roles":[
        {
          "id":"7",
          "name":"Агроном",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"19",
          "name":"Ветеринарный врач",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"43",
          "name":"Зоотехник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"111",
          "name":"Сервисный инженер, инженер-механик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"49",
          "name":"Технолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"21",
      "name":"Спортивные клубы, фитнес, салоны красоты",
      "roles":[
        {
          "id":"8",
          "name":"Администратор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"56",
          "name":"Косметолог",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"60",
          "name":"Массажист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"61",
          "name":"Мастер ногтевого сервиса",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"70",
          "name":"Менеджер по продажам, менеджер по работе с клиентами",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"92",
          "name":"Парикмахер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"138",
          "name":"Фитнес-тренер, инструктор тренажерного зала",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"4",
      "name":"Стратегия, инвестиции, консалтинг",
      "roles":[
        {
          "id":"10",
          "name":"Аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"150",
          "name":"Бизнес-аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"75",
          "name":"Менеджер/консультант по стратегии",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"107",
          "name":"Руководитель проектов",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"134",
          "name":"Финансовый аналитик, инвестиционный аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"22",
      "name":"Страхование",
      "roles":[
        {
          "id":"11",
          "name":"Андеррайтер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"91",
          "name":"Оценщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"122",
          "name":"Страховой агент",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"18",
      "name":"Строительство, недвижимость",
      "roles":[
        {
          "id":"6",
          "name":"Агент по недвижимости",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"14",
          "name":"Архитектор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"154",
          "name":"Брокер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"27",
          "name":"Геодезист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"30",
          "name":"Главный инженер проекта",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"34",
          "name":"Дизайнер, художник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"47",
          "name":"Инженер ПТО, инженер-сметчик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"45",
          "name":"Инженер по охране труда и технике безопасности, инженер-эколог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"46",
          "name":"Инженер по эксплуатации",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"48",
          "name":"Инженер-конструктор, инженер-проектировщик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"59",
          "name":"Маляр, штукатур",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"78",
          "name":"Монтажник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"100",
          "name":"Прораб, мастер СМР",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"102",
          "name":"Разнорабочий",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"107",
          "name":"Руководитель проектов",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"108",
          "name":"Руководитель строительного проекта",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"109",
          "name":"Сварщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"115",
          "name":"Слесарь, сантехник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"143",
          "name":"Электромонтажник",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"10",
      "name":"Транспорт, логистика, перевозки",
      "roles":[
        {
          "id":"159",
          "name":"Бортпроводник",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"21",
          "name":"Водитель",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"31",
          "name":"Грузчик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"39",
          "name":"Диспетчер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"52",
          "name":"Кладовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"58",
          "name":"Курьер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"63",
          "name":"Машинист",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"67",
          "name":"Менеджер по логистике, менеджер по ВЭД",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"81",
          "name":"Начальник склада",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"172",
          "name":"Руководитель отдела логистики",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"131",
          "name":"Упаковщик, комплектовщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"20",
      "name":"Туризм, гостиницы, рестораны",
      "roles":[
        {
          "id":"8",
          "name":"Администратор",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"72",
          "name":"Менеджер по туризму",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"74",
          "name":"Менеджер ресторана",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"76",
          "name":"Менеджер/руководитель АХО",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"89",
          "name":"Официант, бармен, бариста",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"94",
          "name":"Повар, пекарь, кондитер",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"130",
          "name":"Уборщица, уборщик",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"140",
          "name":"Хостес",
          "accept_incomplete_resumes":true,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"12",
      "name":"Управление персоналом, тренинги",
      "roles":[
        {
          "id":"17",
          "name":"Бизнес-тренер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"38",
          "name":"Директор по персоналу (HRD)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"153",
          "name":"Менеджер по компенсациям и льготам",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"69",
          "name":"Менеджер по персоналу",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"171",
          "name":"Руководитель отдела персонала",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"117",
          "name":"Специалист по кадрам",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"118",
          "name":"Специалист по подбору персонала",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"3",
      "name":"Финансы, бухгалтерия",
      "roles":[
        {
          "id":"16",
          "name":"Аудитор",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"154",
          "name":"Брокер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"18",
          "name":"Бухгалтер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"50",
          "name":"Казначей",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"158",
          "name":"Комплаенс-менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"57",
          "name":"Кредитный специалист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"155",
          "name":"Методолог",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"147",
          "name":"Специалист по взысканию задолженности",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"134",
          "name":"Финансовый аналитик, инвестиционный аналитик",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"135",
          "name":"Финансовый директор (CFO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"136",
          "name":"Финансовый контролер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"137",
          "name":"Финансовый менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"142",
          "name":"Экономист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"13",
      "name":"Юристы",
      "roles":[
        {
          "id":"166",
          "name":"Директор юридического департамента (CLO)",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"158",
          "name":"Комплаенс-менеджер",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"145",
          "name":"Юрисконсульт",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        },
        {
          "id":"146",
          "name":"Юрист",
          "accept_incomplete_resumes":false,
          "is_default":false,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    },
    {
      "id":"27",
      "name":"Другое",
      "roles":[
        {
          "id":"40",
          "name":"Другое",
          "accept_incomplete_resumes":true,
          "is_default":true,
          "select_deprecated":false,
          "search_deprecated":false
        }
      ]
    }
  ]
}
```


[link list](https://api.hh.ru/areas)
```
113 Россия
5 Украина
40 Казахстан
9 Азербайджан
16 Беларусь
28 Грузия
1001 Другие регионы
48 Кыргызстан
97 Узбекистан
```


### hh api
```json
{
  "items":[
    {
      "id":"123359231",
      "premium":false,
      "name":"Программист 1С ЗУП",
      "department":null,
      "has_test":false,
      "response_letter_required":false,
      "area":{
        "id":"79",
        "name":"Саратов",
        "url":"https://api.hh.ru/areas/79"
      },
      "salary":null,
      "salary_range":null,
      "type":{
        "id":"open",
        "name":"Открытая"
      },
      "address":null,
      "response_url":null,
      "sort_point_distance":null,
      "published_at":"2025-08-19T11:41:23+0300",
      "created_at":"2025-08-19T11:41:23+0300",
      "archived":false,
      "apply_alternate_url":"https://hh.ru/applicant/vacancy_response?vacancyId=123359231",
      "branding":{
        "type":"MAKEUP",
        "tariff":null
      },
      "show_logo_in_search":true,
      "show_contacts":false,
      "insider_interview":null,
      "url":"https://api.hh.ru/vacancies/123359231?host=hh.ru",
      "alternate_url":"https://hh.ru/vacancy/123359231",
      "relations":[ ],
      "employer":{
        "id":"9582396",
        "name":"ДИДЖИ ТЕХ",
        "url":"https://api.hh.ru/employers/9582396",
        "alternate_url":"https://hh.ru/employer/9582396",
        "logo_urls":{
          "original":"https://img.hhcdn.ru/employer-logo-original/1320507.jpg",
          "90":"https://img.hhcdn.ru/employer-logo/6902074.jpeg",
          "240":"https://img.hhcdn.ru/employer-logo/6902075.jpeg"
        },
        "vacancies_url":"https://api.hh.ru/vacancies?employer_id=9582396",
        "accredited_it_employer":true,
        "trusted":true
      },
      "snippet":{
        "requirement":"Специалиста, который горит 1С:ЗУП и хочет развиваться в этой области. Обязательное требование: наличие сертификата 1С:Специалист по...",
        "responsibility":"Разработка и доработка функционала 1С:ЗУП 8 в рамках проектов внедрения и сопровождения только для одного клиента! "
      },
      "contacts":null,
      "schedule":{
        "id":"remote",
        "name":"Удаленная работа"
      },
      "working_days":[ ],
      "working_time_intervals":[ ],
      "working_time_modes":[ ],
      "accept_temporary":false,
      "fly_in_fly_out_duration":[ ],
      "work_format":[
        {
          "id":"REMOTE",
          "name":"Удалённо"
        }
      ],
      "working_hours":[
        {
          "id":"HOURS_8",
          "name":"8 часов"
        }
      ],
      "work_schedule_by_days":[
        {
          "id":"FIVE_ON_TWO_OFF",
          "name":"5/2"
        }
      ],
      "night_shifts":false,
      "professional_roles":[
        {
          "id":"96",
          "name":"Программист, разработчик"
        }
      ],
      "accept_incomplete_resumes":false,
      "experience":{
        "id":"between1And3",
        "name":"От 1 года до 3 лет"
      },
      "employment":{
        "id":"full",
        "name":"Полная занятость"
      },
      "employment_form":{
        "id":"FULL",
        "name":"Полная"
      },
      "internship":false,
      "adv_response_url":null,
      "is_adv_vacancy":false,
      "adv_context":null
    }
  ],
  "found":1101,
  "pages":23,
  "page":22,
  "per_page":50,
  "clusters":null,
  "arguments":null,
  "fixes":null,
  "suggests":null,
  "alternate_url":"https://hh.ru/search/vacancy?area=113&date_from=19.08.2025&date_to=19.08.2025+23%3A59%3A59&enable_snippets=true&items_on_page=50&page=22&professional_role=96"
}
```
