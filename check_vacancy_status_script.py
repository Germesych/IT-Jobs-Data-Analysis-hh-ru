...
async def test_all_proxies(proxies: List[ProxyConfig]) -> tuple[List[ProxyConfig], List[ProxyConfig]]:
    """Тестирует все прокси и возвращает рабочие и нерабочие прокси"""
...
async def send_proxy_test_report(working_proxies: List[ProxyConfig], failed_proxies: List[ProxyConfig]):
    """Отправляет отчет о тестировании прокси в Telegram"""
    try:
        total_proxies = len(working_proxies) + len(failed_proxies)
        working_count = len(working_proxies)
        failed_count = len(failed_proxies)
        
        message = f"📊 Отчет тестирования прокси:\n"
        message += f"Всего прокси: {total_proxies}\n"
        message += f"✅ Рабочих: {working_count} ({working_count/total_proxies*100:.1f}%)\n"
        message += f"❌ Не рабочих: {failed_count} ({failed_count/total_proxies*100:.1f}%)\n"
        
        if failed_proxies:
            message += f"\n❌ Не рабочие прокси:\n"
            for proxy in failed_proxies:
                message += f"  - {proxy.host}:{proxy.port}\n"
        
        await send_simple_message(message)
        print(f"\n📊 Отчет тестирования прокси отправлен в Telegram")
        
    except Exception as e:
        logger.error(f"Ошибка при отправке отчета прокси: {e}")


async def send_data_collection_started(vacancy_count: int):
    """Отправляет сообщение о начале сбора данных"""
    try:
        message = f"🚀 Запущен сбор данных\n"
        message += f"Количество ID в работе: {vacancy_count}"
        await send_simple_message(message)
        print(f"📨 Сообщение о начале сбора данных отправлено")
    except Exception as e:
        logger.error(f"Ошибка при отправке сообщения о начале сбора: {e}")


async def send_final_report(total: int, closed: int, active: int, errors: int, closed_ids: List[str]):
    """Отправляет финальный отчет в Telegram"""
    try:
        message = f"📊 (TW parser hh) Результаты проверки вакансий:\n"
        message += f"Всего проверено: {total}\n"
        message += f"✅ Закрытых: {closed} ({closed/total*100:.1f}%)\n"
        message += f"🟢 Активных: {active} ({active/total*100:.1f}%)\n"
        message += f"❌ Ошибок: {errors} ({errors/total*100:.1f}%)\n"
        
        if closed_ids:
            message += f"\n📋 Закрытые ID ({len(closed_ids)}):\n"
            # Ограничиваем количество ID в сообщении чтобы не превысить лимит Telegram
            if len(closed_ids) <= 5:
                message += ", ".join(closed_ids)
            else:
                message += f"Первые 5 из {len(closed_ids)}: {', '.join(closed_ids[:20])}"
        
        await send_simple_message(message)
        print(f"📊 Финальный отчет отправлен в Telegram")
        
    except Exception as e:
        logger.error(f"Ошибка при отправке финального отчета: {e}")
...
async def check_vacancies_batch(
    ...
    if test_proxies:
        working_proxies, failed_proxies = await test_all_proxies(proxies)
        
        # Отправляем отчет о тестировании прокси
        await send_proxy_test_report(working_proxies, failed_proxies)
        
        if not working_proxies:
            try:
                message = f'Ошибка проверки закрытых вакансий: ❌ Нет рабочих прокси!!'
                await send_simple_message(message)
            except Exception as err:
                logger.error(f"Ошибка в send_simple_message(): {err}")
    ...
    await send_data_collection_started(len(vacancy_ids))
...
async def main():
    ...
    try:
        message = f'(TW parser hh) Проверка закрытых вакансий началась: {len(vacancy_ids)}!'
        await send_simple_message(message)
    except Exception as err:
        logger.error(f"Ошибка в main send_simple_message(): {err}")
    ...
    try:
        ...
        await send_final_report(total, closed, active, errors, closed_ids)
        ...
        try:
            message = f'🚨 Критическая ошибка в проверке вакансий: {str(e)}'
            await send_simple_message(message)
        except Exception as err:
            logger.error(f"Ошибка при отправке сообщения об ошибке: {err}")
        return []
